	...
	tasks["${boardValue}"] = {
		node {
			...
			sh '''#!/bin/bash
				...
				make ${board}
			'''
			archiveArtifacts "boards/${board}/output_files/*.sof,boards/${board}/output_files/*.rbf,boards/${board}/output_files/*.rpt,boards/${board}/*.qsys,boards/${board}/*.sopcinfo,boards/${board}/hps_isw_handoff/*,boards/${board}/*.dts*"
		}
	}
}
